...
function extractTldData(htmlContent) {
  ...
  categoryDivs.forEach((div) => {
    ...
    tldSpans.forEach(span => {
      ...
      const formattedExtension = extension.startsWith('.') ? extension : `.${extension}`;
...
function main() {
  try {
    ...
    const popularCategory = tldData.categories.find(cat => cat.name === 'Popular');
    if (!popularCategory) {
      throw new Error('Popular category not found in extracted data');
    }
    
    console.log(`Popular category contains ${popularCategory.tlds.length} TLDs`);
